...
class CloseoutController extends KODController
{
    ...
    protected $resend = [22181, 22119, 22101, 22113, 22088, 22133, 22148, 22112, 22156, 22142, 22138, 22116, 22134, 22201, 22192, 22107, 22137, 22159, 22130, 22082, 22084, 22085, 22097, 22115, 22178, 22175, 22093, 22114, 22185, 22079, 22103, 22105, 22172, 22176, 22109, 22143, 22187, 22145, 22089, 22118, 22100, 22146, 22155, 22197, 22078, 22204, 22203, 22191, 22092, 22127, 22126, 22164, 22165, 22167, 22190];
    use EntryHelper;
...
//			$message->to( 'doug@asknice.com', $user->writingName )->subject( 'Thank you for judging this year' );    for a test run
    ...
    public function emailList(){
        foreach($this->resend as $entryID){
            $entry = Entry::with('scoresheets')
                ->find($entryID)->first();
            $this->sendScores($entry,'pubnonfinal');
            sleep(2);
        }
    }
